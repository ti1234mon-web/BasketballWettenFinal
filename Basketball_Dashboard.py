...
# MIT NORMALVERTEILUNG (OHNE SCIPY)
# ============================================================
...
# ============================================================
# 1. HILFSFUNKTIONEN (OHNE SCIPY)
# ============================================================

def normal_cdf(x: float, mu: float = 0.0, sigma: float = 1.0) -> float:
    """
    Kumulative Verteilungsfunktion der Normalverteilung.
    Verwendet math.erf – keine externe Library nötig.
    """
    if sigma <= 0:
...
class BettingCalculator:
    ...
    def match_probabilities(self) -> Dict[str, float]:
        ...
        p_home_win = 1 - normal_cdf(0, mu_diff, sigma_diff)
...
def tab_spread(calc: BettingCalculator):
    ...
    with col1:
        p_unter_165_5 = normal_cumulative_under(165.5, avg, sigma['Sigma'])
        st.metric("Unter 165,5", f"{p_unter_165_5*100:.2f}%")
    with col2:
        p_ueber_150_5 = normal_cumulative_over(150.5, avg, sigma['Sigma'])
        st.metric("Über 150,5", f"{p_ueber_150_5*100:.2f}%")
